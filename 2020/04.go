...
func valid2(f string) bool {
	...
	switch pf {
	case "cm": if err != nil || h < 150 || h > 193 { return false }
	case "in": if err != nil || h < 59 || h > 76 { return false }
	default: return false
	}
	var hcl = field(f, "hcl")
	if len(hcl) != 7 || hcl[0] != '#' { return false }
	_, err = strconv.ParseInt(hcl[1:], 16, 32)
	if err != nil { return false }
	if !ecls[field(f, "ecl")] { return false }
	_, err = strconv.ParseInt(field(f, "pid"), 10, 32)
	if err != nil || len(field(f, "pid")) != 9 { return false }
	return true
}
...
func main() {
	...
	for scanner.Scan() {
		var l = scanner.Text()
		if len(l) == 0 {
			if valid(fields) { res[0]++ }
			if valid2(fields) { res[1]++ }
			fields = ""
		} else {
			fields = fields + " " + l
		}
